...
export const authOptions: NextAuthOptions = {
  providers: [
    CredentialsProvider({
      ...
      // HIER BEGINNT DIE KORRIGIERTE FUNKTION
      async authorize(credentials) {
        ...
        try {
          ...
          // ✅ ==============================================
          // ✅ KORREKTUR: Login-Ereignis protokollieren
          // ✅ ==============================================
          try {
            // HIER IST DIE ÄNDERUNG: ${user.id}::uuid
            await sql`
              INSERT INTO login_logs (user_id, user_email, user_role)
              VALUES (${user.id}::uuid, ${user.email}, ${user.role});
            `;
