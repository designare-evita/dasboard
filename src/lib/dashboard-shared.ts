...
import { ChartEntry, ConvertingPageData } from '@/lib/dashboard-shared'; // ✅ Type Import erweitert
...
// ✅ NEU: Funktion zum Laden der Top Converting Pages
export async function getTopConvertingPages(
  ...
  try {
    const response = await analytics.properties.runReport({
      ...
      requestBody: {
        dateRanges: [{ startDate, endDate }],
        dimensions: [{ name: 'landingPagePlusQueryString' }], // Oder 'pagePath'
        metrics: [
          { name: 'conversions' },
          { name: 'sessions' },
          { name: 'engagementRate' }
