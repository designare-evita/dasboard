...
export default function EditUserForm({ id, user, onUserUpdated }: { id?: string, user: User, onUserUpdated?: () => void }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      setSemrushTrackingId02(user.semrush_tracking_id_02 || '');
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      semrush_tracking_id_02: user.role === 'BENUTZER' ? semrushTrackingId02 : null,
    };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {user.role === 'BENUTZER' && (
          <>
            ...
            {/* Semrush Tracking-ID 02 - NEUES FELD */}
            <div>
              <label className="block text-sm font-medium text-gray-700">
                Semrush Tracking-ID 02
                {semrushTrackingId02 && (
                  <span className="ml-2 text-xs text-green-600">✓ Gesetzt</span>
                )}
              </label>
              ...
              />
              {semrushTrackingId02 && (
                <p className="mt-1 text-xs text-gray-500">Aktueller Wert: {semrushTrackingId02}</p>
              )}
            </div>
          </>
        )}
