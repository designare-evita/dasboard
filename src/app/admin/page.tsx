...
// KEIN Import für 'Button' mehr, um den Fehler zu beheben
import { 
  Pencil, 
  Trash, 
  PersonPlus, 
    ...
    <div className="p-8 max-w-6xl mx-auto bg-gray-50 min-h-screen">
      <div className="mb-8 flex justify-between items-center">
        <div>
         ...
         {/* KORREKTUR: Button-Komponente entfernt, Link mit Tailwind-Klassen verwendet */}
         <Link 
            href="/admin/redaktionsplan" 
            className="bg-white text-gray-700 px-4 py-2 rounded-md hover:bg-gray-50 text-sm font-medium border border-gray-300"
         >
           Zum Redaktionsplan
         </Link>
      </div>
            ...
            <ul className="space-y-3">
              {users.map((user) => (
                  <li key={user.id} className="p-3 border rounded-md flex justify-between items-center transition-colors hover:bg-gray-50">
                    ...
                    {/* KORREKTUR: Button-Komponenten entfernt und zurück zu normalen Tags */}
                    <div className="flex gap-2 flex-shrink-0 ml-4">
                      <Link
                        href={`/admin/edit/${user.id}`}
                        className="bg-gray-200 text-gray-700 px-3 py-1 rounded hover:bg-gray-300 text-sm flex items-center gap-1.5"
                      >
                        <Pencil size={14} /> Bearbeiten
                      </Link>
                      <button
                        onClick={() => void handleDelete(user.id)}
                        className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 text-sm flex items-center gap-1.5"
                      >
