    ...
    return (
      <div className="bg-white p-3 border border-gray-200 shadow-lg rounded-lg text-sm z-50">
        ...
        <div className="space-y-1 text-gray-600">
          <p>🆕 Neue Besucher: <span className="font-medium text-indigo-600">{data.newUsers}</span></p>
          <p>👥 Sitzungen gesamt: <span className="font-medium">{data.sessions}</span></p>
          <hr className="border-gray-100 my-1"/>
          <p>⚡ Interaktionsrate: <span className={`font-medium ${data.engagementRate && data.engagementRate > 50 ? 'text-emerald-600' : 'text-amber-600'}`}>{data.engagementRate}%</span></p>
          <p>🎯 Conversions: <span className="font-medium text-amber-600">{data.conversions}</span></p>
        </div>
      </div>
    );
  }
...
export default function LandingPageChart({ data, isLoading, title = "Top Landingpages" }: Props) {
  ...
  // Sortiere nach Neuen Nutzern für das Diagramm
  const sortedData = [...data].sort((a, b) => (b.newUsers || 0) - (a.newUsers || 0)).slice(0, 7);
         ...
         {sortedData.map((page, i) => (
           <div key={i} className="flex items-center justify-between text-xs py-1 border-b border-gray-50 last:border-0">
              ...
              <div className="flex gap-3 text-right">
                 <span className="w-12 text-indigo-600 font-medium">{page.newUsers} Neu</span>
                 <span className={`w-12 font-medium ${page.engagementRate && page.engagementRate > 60 ? 'text-emerald-600' : 'text-gray-600'}`}>{page.engagementRate}% Eng.</span>
                 <span className="w-8 text-amber-600 font-bold">{page.conversions} ★</span>
              </div>
           </div>
         ))}
      </div>
